...
from model.head import Head, HeadJointPoints
from model.fpn import FPN
from loss.utils import map2coords
import torch
from torch import nn
import torch.nn.functional as F
from dataset.barcode import pol2cart

def poleToCart(idx1, idx2, points, center_x, center_y):
    pole = pol2cart(points[idx1], points[idx2])
    pole[0] += center_x
    pole[1] += center_y
    return (int(pole[0]), int(pole[1]))
...
class CenterNet(nn.Module):
    ...
    # @torch.no_grad()
    # def inference(self, img, infos, topK=40, return_hm=False, th=None, boxes=None):
    ...
    #
    #     keypoints = gather_feature(pred_wh, index, use_transform=True)
    #     keypoints = keypoints.reshape(b, topK, 4)
    #
    ...
    #
    #         batch_keypoints = keypoints[batch][mask.squeeze(-1), :] # boxes[batch]
    #
    #         center_y = ys[batch][mask].cpu()
    #         center_x = xs[batch][mask].cpu()
    #
    #         batch_keypoints[:, [0, 2, 4, 6]] *= infos['raw_width'] / output_w
    #         batch_keypoints[:, [1, 3, 5, 7]] *= infos['raw_height'] / output_h
    #
    #         batch_keypoints *= 1#self.down_stride
    #         print(batch_keypoints)
    #         #boxes_f = detects[batch][2].cpu()
    #
    #         for i in range(batch_keypoints.shape[0]):
    #             idx1 = 0
    #             idx2 = 1
    #             boxes_f_xy_i = []
    #             while idx1 < 8 or idx2 < 8:
    #                 boxes_f_xy_i.append(poleToCart(idx1, idx2, batch_keypoints[i].cpu(), center_x[i].cpu(), center_y[i].cpu()))
    #                 idx1 += 2
    #                 idx2 += 2
    #             boxes_f_xy.append(boxes_f_xy_i)
    #
    ...
    #
    #         detects.append([boxes_f_xy, batch_scores, batch_clses, pred_hm[batch] if return_hm else None])
    #     return detects
    ...
    def inference(self, img, infos, topK=40, return_hm=False, th=None):
        ...
        if self._fpn:
            ...
            feat = feats[-1]
        pred_hm, pred_wh, pred_offset, _ = self.head(self.upsample(feat))
        ...
        for batch in range(b):
            ...
            batch_clses = clses[batch][mask]
            #batch_clses_mask = torch.from_numpy(np.array([self.CLASSES_NAME[int(cls.item())] != "FinderPatterns" for cls in batch_clses]))
            batch_clses = [self.CLASSES_NAME[int(cls.item())] for cls in batch_clses]

            # delete for patterns
            #if len(list(batch_clses_mask.numpy())) != 0:
            #    batch_clses = np.array(batch_clses)[batch_clses_mask.numpy()]
            #    batch_scores = torch.masked_select(batch_scores.cpu(), batch_clses_mask.cpu())
            #    batch_boxes = torch.masked_select(batch_boxes.cpu(), batch_clses_mask.cpu().repeat(1, 4).view(-1, 4)).view(-1, 4)
