...
def calTpFpFn(work_data):
    ...
    tp, fp, fn = getTpFpFn(refBoxes, predBoxes)
...
if __name__ == '__main__':
    for network_id in range(0, 9):
        path = f"/home/artem/PycharmProjects/backboned-unet-new/ckp/f5+100true/{str(network_id)}.pth"
        print(path)
        ckp = torch.load(path)
        ...
        for num, data in tqdm.tqdm(enumerate(dl)):
            ...
            for th_i, th in enumerate( list(np.linspace(0.0, 1/num_th, num_th // 3)) +
                                       list(np.linspace(1/num_th, 1 - 1/num_th, num_th // 3)) +
                                       list(np.linspace(1 - 1/num_th, 1, num_th // 3)) ):
