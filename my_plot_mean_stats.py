...
def gather_data(path, prec_arr, rec_arr):
    ...
    prec_filtered = prec[f1 > np.percentile(f1, 60)]
    rec_filtered = rec[f1 > np.percentile(f1, 60)]
    th_filtered = th[f1 > np.percentile(f1, 60)]

    prec_arr.append(prec_filtered)
    rec_arr.append(rec_filtered)

    max_index = np.argmax(data_plot2[start:-end, 1])
...
def get_interpolation_line(line_x, line_y):
    line_x_indexes = np.argsort(line_x)
    line_x = line_x[line_x_indexes]
    line_y = line_y[line_x_indexes]

    new_line_x = np.linspace(0, 1, 1000)
    new_line_y = np.interp(new_line_x, line_x, line_y)
...
name = "resnetCenterNetUnet+100+syntetic+iou"
display_name = "resnetCenterNetUnet"
plot_data(name, display_name)


name = "f1+100+syntetic+iou"
...
name = "f1+v1+100+syntetic+iou"
display_name = "f1+100+syntetic"
plot_data(name, display_name)

name = "f5+v1+100+syntetic+iou"
display_name = "f5+100+syntetic"
plot_data(name, display_name)
