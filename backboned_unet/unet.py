...
from backboned_unet.config import Config
...
class FilterConv2d(nn.Module):
    def __init__(self,
                 ...
                 padding,
                 bias ):
        super(FilterConv2d, self).__init__()

        print(kernel_size)
        self.mask_x = torch.tensor([range(1, kernel_size[1] + 1) for i in range(kernel_size[1])]).cuda()
        self.mask_y = torch.tensor([ [i] * kernel_size[1]  for i in range(1, 1 + kernel_size[0])]).cuda()

        print(self.mask_y, self.mask_x, kernel_size)
    ...
    def forward(self, x):
        ...
        c_out, c_in, w, h = weights.shape[0], weights.shape[1], weights.shape[2], weights.shape[3]
        for c_out_i in range(c_out):
            for c_in_i in range(c_in):
                idx_weights = torch.topk(weights[c_out_i, c_in_i].flatten(), 2).indices
                idx_y = idx_weights // h
                idx_x = idx_weights % w
                print(idx_y, idx_x)
                print(weights[c_out_i, c_in_i])
                condition = (self.mask_x - idx_x[0]) / (idx_x[1] - idx_x[0]) == (self.mask_y - idx_y[0]) / (idx_y[1] - idx_y[0])
                print((self.mask_x - idx_x[0]) / (idx_x[1] - idx_x[0]))
                print((self.mask_y - idx_y[0]) / (idx_y[1] - idx_y[0]))
                print(condition)
                weights[c_out_i, c_in_i] = torch.where( condition, 0, weights[c_out_i, c_in_i] )
...
class UpsampleBlock(nn.Module):
    ...
    def __init__(self, ch_in, ch_out=None, skip_in=0, use_bn=True, parametric=False):
        ...
        if parametric:
            ...
            #self.conv1 = FilterConv2d(in_channels=ch_in, out_channels=ch_out, kernel_size=(3, 3),
            #                       stride=1, padding=1, bias=(not use_bn))
        ...
        self.conv2 = nn.Conv2d(in_channels=conv2_in, out_channels=ch_out, kernel_size=(3, 3),
                               stride=1, padding=1, bias=(not use_bn))
        #self.conv2 = FilterConv2d(in_channels=ch_in, out_channels=ch_out, kernel_size=(3, 3),
        #             stride=1, padding=1, bias=(not use_bn))
...
if __name__ == "__main__":
    # simple test run
    net = Unet( backbone_name=Config.slug )

    if Config.resume:
        net.backbone.load_state_dict( torch.load( Config.backbone_checkpoint_dir ) )
